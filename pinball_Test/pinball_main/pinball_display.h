...
void ClearDispDigit(int digit) { // digits 0-7, score 0-5, ball 6, cradit 7
  ...
  if (digit == 1 || digit == 3 || digit == 5 || digit == 7) { //4 bit offset if true
    for (int bito = 4; bito < 8; bito++) { //now have 4 bits to reset
      digitalWrite(dispBitSel[0], dispHL[bitMap[bito][0]]);
      digitalWrite(dispBitSel[1], dispHL[bitMap[bito][1]]);
      digitalWrite(dispBitSel[2], dispHL[bitMap[bito][2]]);
    ...
    for (int bit = 0; bit < 4; bit++) { //now have 4 bits to reset
      digitalWrite(dispBitSel[0], dispHL[bitMap[bit][0]]);
      digitalWrite(dispBitSel[1], dispHL[bitMap[bit][1]]);
      digitalWrite(dispBitSel[2], dispHL[bitMap[bit][2]]);
      digitalWrite(dispLatchEnable, LOW); //Write one bit
      delay(stdDispLatchDelay);
      digitalWrite(dispLatchEnable, HIGH);
    }
...
void SetDispDigit(int digit, int value) { // digits 0-7, score 0-5, ball 6, cradit 7. Value must be single digit here.
  ...
  if (digit == 1 || digit == 3 || digit == 5 || digit == 7) { //4 bit offset if true
    for (int bito = 4; bito < 8; bito++) { //now have 4 bits to set
      digitalWrite(dispBitSel[0], dispHL[bitMap[bito][0]]); //each digit is set by 4 bits
      digitalWrite(dispBitSel[1], dispHL[bitMap[bito][1]]); //but each of those bits have their own 3-bit address!
      digitalWrite(dispBitSel[2], dispHL[bitMap[bito][2]]); //each iteration picks another addr. from bitMap[][]

      digitalWrite(dispDataLn, dispHL[decimal2BCD[value][bito-4]]); //picks one of the 4 bit values for this particular decimal digit value
